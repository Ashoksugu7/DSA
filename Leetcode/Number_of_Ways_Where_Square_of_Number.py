...
class Solution:
    def val_list_count(self, val, lister):
        count=0
        for i in range(len(lister)-1):
            for j in range(i+1, len(lister)):
                if val*val==(lister[i]*lister[j]):
                    # print(val, lister[i], lister[j])
                    count+=1
        return count
